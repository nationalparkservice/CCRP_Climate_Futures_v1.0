...
# Function to subset DFs to GCMs
...
Baseline_all<-GCMsubset(Baseline_all)
Future_all<-GCMsubset(Future_all)
WB<-GCMsubset(WB)
...
GCMorder<-c("GFDL-ESM2G.rcp85", "MIROC-ESM.rcp85", "CSIRO-Mk3-6-0.rcp45", "HadGEM2-ES365.rcp85")

#X-axis labels for monthly plots
MonthLabels = c("J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D")

set.seed(13)
H_samp<-sample_n(H_annual,30); H_samp$GCM<-"Historical"
Annual<-rbind(H_samp,F_annual)
Annual$GCM<-factor(Annual$GCM,levels = c(GCMorder,"Historical"))
Annual <- Annual[c(2,1,3:30)] # change column placement so mirrors WICA script
...
CFnames <- c("Climate Future 1","Climate Future 2","Climate Future 3","Climate Future 4")
