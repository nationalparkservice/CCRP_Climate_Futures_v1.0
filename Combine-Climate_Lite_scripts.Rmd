...
```{r PRISM-plots, results=hide, eval=FALSE} 
...
```{r Copy-and-delete-files, eval=FALSE}
...
rfile_list <- list.files(project_root_dir, pattern = '.Rmd') 

scripts <- paste(project_root_dir, "scripts", sep = "/")

# Create folder structure that mimics 

scripts_list <- list.files(paste(project_root_dir, "scripts", sep = "/"), pattern = '.R', recursive = TRUE, include.dirs = TRUE)
file.copy(file.path(project_root_dir, rfile_list), local_rss_dir)
