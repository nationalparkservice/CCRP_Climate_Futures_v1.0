...
final <- rbind(H_annual, F_annual)

summary <- final %>%
  group_by(CF) %>%
  summarise(across(ends_with("Custom"), list(min = min, mean = mean, max = max, sd = sd))) 

summary2 <- final %>%
  group_by(CF) %>%
  summarize(across(starts_with("Over"), list(min = min, mean = mean, max = max, sd = sd)))

summary3 <- full_join(summary, summary2)
...
summary2_old <- final_old %>%
  group_by(CF) %>%
  summarize(across(starts_with("Over"), list(min = min, mean = mean, max = max, sd = sd)))

summary3_old <- full_join(summary, summary2)

# Compare

m1 <- data.matrix(summary3)
m2 <- data.matrix(summary3_old)
