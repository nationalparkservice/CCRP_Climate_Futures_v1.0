...
load("C:/Users/adillon/Documents/RSS/DINO/MACA_S/Figs/DINO-S_T2.RData")
...
Gridmet<-read.csv("C:/Users/adillon/Documents/RSS/DINO/MACA_S/GridMet.csv",header=T)
...
colors5 <-   c("white","#12045C","#9A9EE5","#ffcccc","#E10720")
...
Gridmet$Month<-month(Gridmet$Date)
Gridmet$Year<-year(Gridmet$Date)
Gridmet$tmean_C<-(((Gridmet$tmax+Gridmet$tmin)/2)-32)*5/9
Gridmet$Pr_mm<-Gridmet$precip*25.4

Gridmet$daylength <- get_daylength(Gridmet$Date, lat = Lat) # get daylength for use with Hamon PET equation
Gridmet$PET <- ET_Hamon_daily(Gridmet)

d<-aggregate(Pr_mm~Month+Year,Gridmet,sum)
d2<-aggregate(tmean_C~Month+Year,Gridmet,mean)
d3 <- aggregate(PET~Month+Year, Gridmet, mean) # get mean Hamon PET for month

drt<-merge(d,d2, by=c("Month","Year"));rm(d,d2)
drt <- merge(drt,d3, by = c("Month", "Year")); rm(d3) 
drt <- drt[,1:5]
colnames(drt) <- c("Month", "Year", "Pr_mm", "TmeanC", "PET")
...
ALL$tmean_C<-(ALL$TavgCustom-32)*5/9 # tmean_C required for use with ET_Hamon_daily fxn
ALL$daylength <- get_daylength(ALL$Date, lat = Lat) # for Hamon PET
ALL$PET <- ET_Hamon_daily(ALL)

M<-aggregate(Pr_mm~Month+Year+GCM,ALL,sum)
M2 <- aggregate(PET ~ Month+Year+GCM, ALL, mean)
M3 <- aggregate(tmean_C~Month+Year+GCM,ALL,mean)
Mon<-merge(M,M2,by=c("Month","Year","GCM"))
Mon <- merge(Mon, M3, by = c("Month", "Year","GCM"))
...
CF3<-subset(all3, CF %in% CFs[3] )
grid.append<-drt3; grid.append$CF<-CFs[3]
grid.append<-subset(grid.append, select=c(Year,CF,Pr_mm:col))
grid.append<-rbind(grid.append, subset(CF2,Year>=2020 & Year < 2070))

CF4<-subset(all3, CF %in% CFs[4] )
grid.append<-drt3; grid.append$CF<-CFs[4]
grid.append<-subset(grid.append, select=c(Year,CF,Pr_mm:col))
grid.append<-rbind(grid.append, subset(CF2,Year>=2020 & Year < 2070))
