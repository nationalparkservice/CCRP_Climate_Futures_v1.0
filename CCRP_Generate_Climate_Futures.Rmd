...
library(gridExtra); library(SPEI); library(tidyr); library(tibble); library(sp); library(skimr); library(cft); library(stringr)
...
local_rss_dir <- 'C:/Users/achildress/OneDrive - DOI/CF_management/RCF_cloud-run_rebuild/Testing/' # local directory where you want this repo copied. Should be something like 'C:/Users/adillon/Documents/RSS/'. Next lines create folder for park
...
OutDir = './data/park-specific/output' # for .csv's
...
# source(here::here("scripts", "WaterBalance", "get_params.R")) # Extract WB site parameters; 
...
ww_hd <- paste0(local_rss_dir,"/WarmWet_HotDry")
dir.create(ww_hd)
...
if(centroids_csv == "Y") {
  source(here::here("scripts", "Climate-Futures", "download_Tercek.csvs.R"), local = knitr::knit_global())
} else {
...
source(here:here("scripts", "Additional-tables-plots", "Return_Events"))
...
source(here::here("scripts", "Misc", "var_summary_stats.R")) #read in output/Plot_data for each CF
